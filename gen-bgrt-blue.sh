...
RENDERDIR=render-bgrt-blue
TARGETDIR=legion-bgrt-blue
...
#Remove old Target Directory and create new one
[ -d "$TARGETDIR" ] && rm $TARGETDIR
mkdir $TARGETDIR
...
for file in "${RENDERDIR_LIST[@]}"; do
	filenum=${file%.png}
	filenum=$((10#$filenum))

	if [ $FRAME_THROBBER_START -le "$filenum" ] && [ $FRAME_THROBBER_END -ge "$filenum" ]; then
	...
	elif [ $FRAME_STARTUP_ANIM_START -le "$filenum" ] && [ $FRAME_STARTUP_ANIM_END -ge "$filenum" ]; then
		target_filenum=$((filenum - FRAME_STARTUP_ANIM_START + 1))
		target_filenum=$(printf "%04d" $target_filenum)
		target_filename="${STARTUP_ANIM_PFX}${target_filenum}.png"
	elif [ $FRAME_SHUTDOWN_ANIM_START -le "$filenum" ] && [ $FRAME_SHUTDOWN_ANIM_END -ge "$filenum" ]; then
...
magick -delay 1x30 $TARGETDIR/$THROBBER_PFX*.png $TARGETDIR/$STARTUP_ANIM_PFX*.png -background black -alpha remove -gravity center -extent 1280x720 -layers optimize -loop 0 startup-bgrt-blue.gif

echo "Saved startup.gif"

magick -delay 1x30 $TARGETDIR/$THROBBER_PFX*.png $TARGETDIR/$SHUTDOWN_ANIM_PFX*.png -background black -alpha remove -gravity center -extent 1280x720 -layers optimize -loop 0 shutdown-bgrt-blue.gif
